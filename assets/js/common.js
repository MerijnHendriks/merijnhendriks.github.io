...
class CustomRenderer extends marked.Renderer
{
    ...
    blockquote(quote)
    {
        return `<div class="blog-container px-4 py-2"><blockquote class="blockquote">${quote}</blockquote></div>`;
    }

    table(header, body)
    {
        return `<div class="blog-container p-1"><table class="table"><thead">${header}</thead><tbody>${body}</tbody></table></div>`;
    }
    ...
    code(code, language)
    {
        ...
        return `<div class="blog-container p-3"><pre class="mb-0"><code class="highlight language-${language}">${highlighted}</code></pre></div>`;
    }
}
